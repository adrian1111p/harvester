...
public sealed partial class SnapshotRuntime
{
    ...
    private readonly ILogger<SnapshotRuntime> _logger;
    ...
    public SnapshotRuntime(AppOptions options, IStrategyRuntime? strategyRuntime = null, ILogger<SnapshotRuntime>? logger = null)
    {
        _options = options;
        ...
        _logger = logger ?? NullLogger<SnapshotRuntime>.Instance;
    }

    public async Task<int> RunAsync()
    {
        _logger.LogInformation("Mode={Mode} host={Host}:{Port} clientId={ClientId}", _options.Mode, _options.Host, _options.Port, _options.ClientId);
        ...
        if (runtimeStateStore.TryLoadLatest(out var restoredCheckpoint, out var restoreMessage))
        {
            _logger.LogInformation(
                "Restored runtime checkpoint mode={Mode} exitCode={ExitCode} state={State} checkpoint={CheckpointUtc}",
                restoredCheckpoint!.Snapshot.Mode,
                restoredCheckpoint.Snapshot.ExitCode,
                restoredCheckpoint.Snapshot.ConnectionState,
                restoredCheckpoint.CheckpointUtc);
        }
        else if (!string.IsNullOrWhiteSpace(restoreMessage))
        {
            _logger.LogWarning("{RestoreMessage}", restoreMessage);
        }
        ...
        try
        {
            ...
            _logger.LogInformation("nextValidId={NextValidId}", await _wrapper.NextValidIdTask);
            _logger.LogInformation("managedAccounts={ManagedAccounts}", await _wrapper.ManagedAccountsTask);
            ...
            _logger.LogInformation("currentTime callback received");
            ...
            if (_options.MonitorUiEnabled && _options.Mode != RunMode.PositionsMonitorUi)
            {
                ...
                _logger.LogInformation("Positions monitor dashboard auto-started url={Url}", $"http://localhost:{monitorPort}");
            }
            ...
            {
                _logger.LogWarning("Completed with blocking API errors.");
                PrintErrors();
                PrintRequestDiagnostics();
                if (_hasReconciliationQualityFailure)
                {
                    _logger.LogWarning("Completed with reconciliation quality gate failure.");
                }
                if (_hasConnectivityFailure)
                {
                    _logger.LogWarning("Completed with connectivity halt escalation.");
                }
                if (_hasPreTradeHalt)
                {
                    _logger.LogWarning("Completed with pre-trade control HALT action.");
                }
                if (_hasClockSkewFailure)
                {
                    _logger.LogWarning("Completed with broker clock-skew gate failure.");
                }
                return await FinalizeRunAsync(
                    ...
                    runStartedUtc,
                    exitCode: 1,
                    targetStage: RuntimeLifecycleStage.Halted,
                    lifecycleReason: "blocking error or gate failure",
                    completionMessage: "[FAIL] Completed with blocking API errors.",
                    printErrors: true);
            }
            ...
            return await FinalizeRunAsync(
                ...
                runStartedUtc,
                exitCode: 0,
                targetStage: RuntimeLifecycleStage.Shutdown,
                lifecycleReason: "completed without blocking errors",
                completionMessage: "[PASS] Completed successfully.");
        }
        catch (OperationCanceledException) when (_hasConnectivityFailure)
        {
            ...
            return await FinalizeRunAsync(
                ...
                exitCode: 1,
                targetStage: RuntimeLifecycleStage.Halted,
                lifecycleReason: "connectivity halt escalation",
                completionMessage: "[FAIL] Connectivity halt escalation triggered.",
                printErrors: true,
                notifyModeFailed: true);
        }
        catch (TimeoutException ex)
        {
            ...
            return await FinalizeRunAsync(
                ...
                exitCode: 2,
                targetStage: RuntimeLifecycleStage.Halted,
                lifecycleReason: $"timeout: {ex.Message}",
                completionMessage: $"[FAIL] {ex.Message}",
                printErrors: true,
                notifyModeFailed: true);
        }
        catch (Exception ex)
        {
            ...
            return await FinalizeRunAsync(
                ...
                exitCode: 2,
                targetStage: RuntimeLifecycleStage.Halted,
                lifecycleReason: $"exception: {ex.Message}",
                completionMessage: $"[FAIL] {ex.Message}",
                printErrors: true,
                notifyModeFailed: true);
        }
    ...
    private async Task<int> FinalizeRunAsync(
        StrategyRuntimeContext strategyContext,
        RuntimeStateStore runtimeStateStore,
        IbkrSession session,
        DateTime runStartedUtc,
        int exitCode,
        RuntimeLifecycleStage targetStage,
        string lifecycleReason,
        string? completionMessage = null,
        bool printErrors = false,
        bool notifyModeFailed = false)
    {
        ExportAdapterTraceArtifact();
        ExportStrategySchedulerArtifact();
        ExportResilienceDrillAcceptanceArtifact(session, runStartedUtc, exitCode);
        ExportLeanZiplineParityChecklistArtifact(runStartedUtc, exitCode);
        ExportPromotionReadinessArtifact(runStartedUtc, exitCode);

        if (notifyModeFailed)
        {
            await NotifyStrategyScheduledEventAsync("mode-failed", strategyContext, CancellationToken.None);
        }

        await NotifyStrategyShutdownAsync(strategyContext, exitCode);
        TransitionLifecycle(targetStage, lifecycleReason);

        if (!string.IsNullOrWhiteSpace(completionMessage))
        {
            if (exitCode == 0)
            {
                _logger.LogInformation("{Message}", completionMessage);
            }
            else
            {
                _logger.LogError("{Message}", completionMessage);
            }
        ...
        PersistRuntimeState(runtimeStateStore, session, exitCode, runStartedUtc);
        return exitCode;
    }
    ...
    private void TransitionLifecycle(RuntimeLifecycleStage next, string reason)
    {
        ...
        _logger.LogInformation("Lifecycle transition {Previous} -> {Next} reason={Reason}", previous, next, reason);
    }
        ...
        CancellationToken token)
    {
        ...
        foreach (var batch in symbols.Chunk(batchSize))
        {
            ...
            // Wait for IBKR to deliver bars â€” use adaptive delay: 5s base + 1s per 10 symbols in batch
    ...
    // â”€â”€ V2 Self-Learning Bias Store persistence â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    private static Dictionary<string, SelfLearningV2SymbolBiasRow>? LoadSelfLearningV2BiasStore(string path)
    {
        ...
        {
            // [D9 FIX] Log instead of silently swallowing â€” corrupt store
    ...
    {
        var nextDailyOrderCount = Volatile.Read(ref _dailyTransmittedOrderCount) + 1;
    ...
    {
        Interlocked.Increment(ref _dailyTransmittedOrderCount);
    }
